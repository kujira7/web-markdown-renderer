...
const source = normalizeSourceText(wellFormedMarkdown);
assert.equal(source, wellFormedMarkdown);
...
const indentedCode = normalizeSourceText(`    const x = 1;
    console.log(x);`);
assert.equal(indentedCode, `    const x = 1;
    console.log(x);`);

const nestedList = normalizeSourceText(`- parent

  - child`);
assert.equal(nestedList, `- parent

  - child`);

const blockquoteParagraphs = normalizeSourceText(`> first paragraph
>
> second paragraph`);
assert.equal(blockquoteParagraphs, `> first paragraph
>
> second paragraph`);
